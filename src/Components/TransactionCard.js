            ...
            >
              <div style={{ display: 'flex', width: '100%', padding: '1em'}}>
                <div style={{ padding: '0.5em 1em', width: '50%'}} >
                  <h3>
                    TxHash: <Link to={`/tx/${tx.hash}`} style={{ textDecoration: 'none', color: 'black' }} >{tx.hash}</Link>
                  </h3>
                </div>
              ...
              <div style={{ display: 'flex', width: '100%', padding: '1em', marginBottom: '1em'}}>
                <div style={{ padding: '0.5em 1em',  width: '50%'}} >
                  {tx.timeStamp !== undefined ? <h3>Transaction Date: {getDate(tx.timeStamp)}</h3> : <h3>Nonce: {hexToNumber(tx.nonce)}</h3>}
                </div>
